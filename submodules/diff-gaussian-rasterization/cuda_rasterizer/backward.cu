	...
	bool antialiasing)
{
	...
	if(antialiasing)
	{
		constexpr float min_h_scaling = 0.3f;
		const float det_cov = c_xx * c_yy - c_xy * c_xy;
		c_xx += h_var;
		c_yy += h_var;
		const float det_cov_plus_h_cov = c_xx * c_yy - c_xy * c_xy;
		const float raw_h_scaling = sqrt(max(0.000025f, det_cov / det_cov_plus_h_cov));
		const float h_convolution_scaling = max(min_h_scaling, raw_h_scaling);
		...
		// When clamped (raw < min), gradient through covariance is zero
		const bool clamped = raw_h_scaling < min_h_scaling;
		d_inside_root = (clamped || (det_cov / det_cov_plus_h_cov) <= 0.000025f) ? 0.f : d_h_convolution_scaling / (2 * h_convolution_scaling);
	}
	else
	{
		c_xx += h_var;
