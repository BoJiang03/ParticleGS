	...
	bool antialiasing)
{
	...
	if(antialiasing)
	{
		...
		h_convolution_scaling = max(min_h_scaling, sqrt(max(0.000025f, det_cov / det_cov_plus_h_cov)));
	}
	...
	float* __restrict__ invdepth)
{
    ...
    // Use thrust::maximum to avoid CUB API changes across CUDA versions
    last_contributor = BlockReduce(temp_storage).Reduce(last_contributor, thrust::maximum<uint32_t>());
