...
def main():
    ...
    # ── 8. Raw→VTP conversion time ───────────────────────────────────────
    # Timed once by ensure_shared_data() when the VTP is first created and
    # persisted to runs/shared/timings.json (the VTP is cached afterwards,
    # so it cannot be re-measured here without redoing the conversion).
    print("\n" + "="*70)
    print("[8] Raw→VTP Conversion Time")
    print("="*70)
    timings_path = SHARED_DIR / "timings.json"
    vtp_s = None
    if timings_path.exists():
        vtp_s = json.loads(timings_path.read_text()).get("vtp_conversion_s")
    if vtp_s is not None:
        results["vtp_conversion"] = {
            "vtp_conversion_s": vtp_s,
            "vtp_conversion_min": round(vtp_s / 60, 2),
            "source": "measured_during_shared_prep",
        }
        print(f"  Raw→VTP (280M particles): {vtp_s/60:.2f} min")
    else:
        results["vtp_conversion"] = {
            "vtp_conversion_s": None,
            "vtp_conversion_min": None,
            "source": "unavailable_shared_data_predates_timing",
        }
        print("  Not recorded — runs/shared/ was generated by an artifact "
              "version without timing support.")
